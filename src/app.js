const merchent_key = '6BFPjU';
const secret_Key = 'K0heTR9JiwtnDrcFdWYc83IsaLY6t1d4';
...
// this will send the required data to the user side;
app.get('/test' , (req,res)=>{
  ...
  var txn = JSON.stringify(Math.random()*1000);
   ...
   if(!config.sample_data.mtx || !config.sample_data.amount || !config.sample_data.productinfo
        || !config.sample_data.name || !config.sample_data.email_id){
    return  res.send('something went wrong with data.');
   }else{
...
app.post('/payment/payumoney/response' , (req, res)=>{
  var pd = req.body;
  console.log('got this from response' , pd);
   //Generate new Hash
    var hashString = config.secretkey + '|' + pd.status + '||||||||||' + '|' + pd.email + '|' + pd.firstname + '|' + pd.productinfo + '|' + pd.amount + '|' + pd.txnid + '|' + config.accesskey;

    var cryp = crypto.createHash('sha256' , config.secretkey);
    cryp.update(hashString);
    let hash = cryp.digest('hex');
    console.log(hash , pd.hash);
...
// it will be called from user side to validate the authenticity of the data;
app.post('/payment/payumoney',(req , res)=>{
  ...
  if(!req.body.txnid || !req.body.amount || !req.body.productinfo
       || !req.body.firstname || !req.body.email) {
         ...
         var pd = req.body;
         var hashString = config.accesskey // Merchant Key
                  + '|' + pd.txnid
                  + '|' + pd.amount + '|' + pd.productinfo + '|'
                  + pd.firstname + '|' + pd.email + '|'
                  + '||||||||||'
                  + config.secretkey // Your salt value
          var cryp = crypto.createHash('sha256',config.secretkey);
          cryp.update(hashString);
        	let hash = cryp.digest('hex');
...
function create_hash(data,accesskey,secretkey){
  ...
  // console.log('creating hash ' , hash_string);
...
app.get('/success' , (req,res)=>{
  res.send('its calling');
})
app.get('/failure' , (req,res)=>{
  res.send('its working but something is up;')
})
// virtual league
